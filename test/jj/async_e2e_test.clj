...
(def handler
  (router/async-route {"/"             [(GET (fn [req res rej]
                                               (res {:status 200
                       ...
                       "/delete/:user" [(DELETE (fn [req res rej]
                                                  (res {:headers {"bar" "baz"}
                                                        :status  201
...
(deftest test-request
  (let [server (server/run-http-server handler {:port 8888 :async? true})
